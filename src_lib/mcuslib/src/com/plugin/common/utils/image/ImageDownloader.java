...
public class ImageDownloader extends FileDownloader implements Runnable, Destroyable, HttpRequestHookListener {
    ...
    public static final class ImageFetchRequest extends DownloadRequest {
        ...
        public String toString() {
            return "ImageFetchRequest [mCategory=" + mCategory + ", mBitmapOperationListener="
                    + mBitmapOperationListener + ", mDownloadUrl=" + mDownloadUrl + ", mUrlHashCode=" + mUrlHashCode
                    + ", mType=" + mType + ", mStatus=" + mStatus + "]";
        }
    ...
    public static final class ImageFetchResponse extends DownloadResponse {
        ...
        private ImageFetchResponse(Bitmap bt, String cachePath, String downloadUrl, String rawPath,
        ...
        @Override
        public String toString() {
            return "ImageFetchResponse [mBt=" + mBt + ", mLocalCachePath=" + mLocalCachePath + ", mDownloadUrl="
                    + mDownloadUrl + ", mRawLocalPath=" + mLocalRawPath + ", mRequest=" + mRequest + "]";
        }
    ...
    protected DownloadResponse tryToHandleDownloadFile(String downloadLocalPath, DownloadRequest requestOrg) {
        ...
        if (android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
            try {
                ...
                if (request.mBitmapOperationListener != null) {
                    ...
                    Bitmap downloadBt = mCacheManager
                            .getResource(DEFAULT_RAW_IMAGE_CATEGORY, request.getmDownloadUrl());
                    if (downloadBt != null) {
        ...
        if (bt != null) {
            ImageFetchResponse response = new ImageFetchResponse(bt, localPath, request.getmDownloadUrl(),
                    downloadLocalPath, request);
